...
<?php
    if ( $_GET['start']){
        $startDay = explode(" ", $_GET['start'])[1];
        $startClass = explode(" ", $_GET['start'])[0];
    }
    if (isset($_GET['class'])) {
        // sql管理
        $class = $_GET['class'];
...
<body id="page-top">
    <?php
        if ($user->warning>=3){
            print "<script>alert('同學你警告已達三次了內')</script>";
            header("Refresh: 0.1 ;URL = lend.php?class=$class");
            exit;
        }
    ?>
    <!-- Page Wrapper -->
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
                    ...
                    <ul class="navbar-nav ml-auto">
                        ...
                        <li class="nav-item dropdown no-arrow">
                            <a class="nav-link dropdown-toggle" href="personalInfo.php" id="userDropdown" role="button"
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800"><?=$class?>教室借用</h1>
                    </div>
                        ...
                        <div class="col-lg-12 mb-4">
                            ...
                            <!-- Illustrations -->
                                ...
                                <form action="addClass.php" method="post" > 
                                    <input type="hidden" id="class" value="<?= $class?>" name="class" >
                                    <div class="form-group">
                                        <lebel for="startTime">開始借用時間</lebel>
                                        <?php
                                            $day = match($startDay){
                                                '0' => 'MON',
                                                '1' => 'TUE',
                                        ...
                                        ?>
                                        <input type="text" name = "startTime" class="form-control" id="startTime" value="<?php print $day." ".$lesson ?>"   readonly>                                    
                                    </div>
                                    <div class="form-group">
                                        <lebel for="endTime">借用結束時間</lebel>

                                        <select class="form-control" id="endTime" name = "endTime" required>
                                            <?php
                                                for ($i = intval($startClass ); $i <10; $i++ ){
                                                    if ($tableForLend[$i][$startDay] == 0 && $userTable[$i][$startDay] == 0 ){
                                                        $str=match($startDay){
                                                            '0' => 'MON',
                                                            '1' => 'TUE',
                                                            '2' => 'WED',
